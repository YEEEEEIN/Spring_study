...
public class example04 {
	...
	/*
	 <form-login> 태그
	 : 인증되지 않는 사용자가 특정 경로에 접근할 때 인증이 필요하면 로그인 페이지를 보여줌
	 ...
	 logout 태그
	 - delete-cookies : 로그아웃에 성공할 때 삭제할 쿠키의 이름
	 - invalidate-session : 로그아웃할 때 세션을 제거할지 말지, default : true(제거)
	 - logout-success-url : 로그아웃 하고 이동할 페이지, default : /login?logout
	 - logout-url : 로그아웃 요청 -> 처리할 경로
	 - success-hander-ref : 로그아웃 성공할 때 이동을 제어하는 핸들러
